...
It provides a primary `カレンダー` mode with `日・週・月`, plus `タスク` and `方向` aggregates. Day uses a narrow Apple Calendar-style timeline with a persisted `Elastic | 24時間` scale and a right mini-calendar/properties inspector. Week uses a vertically scrolling 24-hour grid with fixed headers; month is an overview. Calendar blocks represent only actual Flow and rests; Todo completion remains in Task summaries and Statistics. Historical Flow and rest entries open their canonical editors. Historical Flow changes must adjust Direction totals and measured Todo progress by the same delta.
