...
It provides a primary `カレンダー` mode with `日・週・月`, plus `タスク` and `方向` aggregates. Day uses a narrow Apple Calendar-style timeline with a persisted `Elastic | 24時間` scale and a right mini-calendar/properties inspector. Week uses a vertically scrolling 24-hour grid with fixed headers; month is an overview. Only actual Flow, rests, exact Task completions, and legacy untimed completions appear. Pending scheduled Tasks remain in `タスク`. Historical Flow, rest, and Task entries open their canonical editors. Historical Flow changes must adjust Direction totals and measured Todo progress by the same delta.
