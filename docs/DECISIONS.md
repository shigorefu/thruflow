...
The `日` History range directly renders every actual Flow and rest record for the selected app day as a chronological vertical timeline. Cards have a stable interactive size independent of recorded duration, and internal gaps of at least one hour are represented by a quiet, vertically spacious `記録なし` row. There is no `Elastic | 24時間` choice in this view. Selecting a record pushes its canonical editor within the same primary surface; Back returns to the day timeline with the reverse transition. A right pane retains the wide-layout date mini-calendar and selected-record context.
