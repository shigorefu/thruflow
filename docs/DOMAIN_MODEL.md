...
`FlowBreak` persists explicit rest between Flow sessions using stable session UUID references. It stores the series ID, previous/next session IDs, rest start, timer-stop, connection, optional adjusted-end timestamps, planned duration, and `長休憩` state. Sessions started within planned rest × 1.5 share the same series ID. A `長休憩` lasts 20 minutes after every 4 accumulated Blocks in a series and permits continuation for 30 minutes from rest start. `FlowBreakEditor` applies manual duration corrections and pushes only overlapping downstream records from the same series.
