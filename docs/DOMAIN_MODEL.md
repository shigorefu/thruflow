...
`DayHistoryBuilder` produces daily Task/Direction aggregates. `HistoryCalendarBuilder` projects only actual FlowSession, FlowSegment, and FlowBreak records into date-range calendar items without persistence; Todo completion remains aggregate data rather than a calendar item. `HistoryCalendarTrackBuilder` groups projected items by `seriesID`, so work and rest in one Flow series render as one continuous track while separate series remain separate. `HistoryDayTimelineWindowBuilder` derives the day view's Elastic/full-day hour range, while `HistoryOverlapLayout` assigns lanes to colliding actual or minimum-visual intervals independently from SwiftUI. `FlowHistoryEditor` creates independent manual Flow records and corrects Direction and measured Todo totals when a historical Flow is changed or deleted.
