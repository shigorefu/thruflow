...
```text
Area -> Task -> Flow -> actual focused time -> progress -> statistics
```

## Included

- Area management for `いつでも`, `習慣`, and `できたら`.
- System Area `その他`, hidden only from Area management so it cannot be edited.
- Daily `タスク` list.
...
- Canonical `履歴` with Flow calendar and Task/Area aggregates for `日`, `週`, and `月`.
...
- Flow-first iPhone and iPad app with the animated stream above the complete player,
  ...
  five-item material navigation surface: `Flow`, `タスク`, `履歴`, `分野`, and
...
- iPhone Flow dashboard parity for the actionable surfaces: the Task card can
  switch between `タスク / 習慣 / できたら`, provides inline quick capture next
...
- iPhone live quick-input suggestions, arbitrary and no-date Tasks, overdue and
  ...
  editing, ordered Area groups, and a dedicated Area emoji picker.
...
- iPhone Live Activity and Dynamic Island player for an active Flow, including
  system-updating time/progress, Task and Area context, expanded
...
- iPhone Home Screen and macOS desktop `Flowタイマー` widgets in Small and Medium sizes, with
  system-updating time/progress, active Task and Area context, an empty
