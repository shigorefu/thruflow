...
Hovering a dashboard timeline segment shows an immediate compact card with Task, clock interval, and focused duration. Clicking opens an anchored segment popover with Task, Direction, interval, focused duration, and Flow size. Completed segments can continue to the canonical Flow history inspector; the active segment is read-only and marked `実行中`.
