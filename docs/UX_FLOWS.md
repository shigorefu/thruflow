...
On macOS, `+` sits in the top-right History toolbar immediately after Search and opens the shared record form in the system trailing inspector. Clicking an empty time in the weekly calendar opens the same inspector with that day and time preselected. On iOS, the persistent solid-blue `+` remains at the bottom trailing edge of `履歴`. Both platforms use the shared Flow-style target picker with `タスク`, `習慣`, and `方向` tabs. Task lists existing occurrences for the selected day and offers new Task creation. Habit lists every eligible Habit Direction for that day; when no Todo occurrence exists, saving materializes the historical occurrence from the Direction template. Direction records focused work without a linked Todo. The selected Task/Habit card previews its unit and resulting progress before saving.
