...
Inside the Flow task picker, quick creation opens as a separate compact trailing popover. It shows only the messenger composer, without the syntax legend. On macOS this surface is presented by a native `NSPopover` attached as a child of the picker window. The parent also disables interactive dismissal while the composer is visible. This keeps first-responder changes, menus, and buttons inside the same window hierarchy, so interacting with the messenger does not close the menu-bar player or its task picker.
