...
Inside the Flow task picker, quick creation opens as a separate compact trailing popover. It shows only the messenger composer, without the syntax legend. Before presentation, macOS editing is ended and presentation is deferred to the next main-actor turn so AppKit can transfer first responder safely in both the main window and menu bar. While the messenger popover is open, interactive dismissal of its parent picker is disabled, so controls inside the messenger do not close the picker hierarchy.
