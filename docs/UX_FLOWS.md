...
Quick capture behaves like a messenger composer. The user can set measurement, Direction, priority, date, and multiple hashtags from compact controls. The default measurement control reads `種類`; leaving it untouched creates a Check Task. It is one stable animated control: its leading icon distinguishes Check, Block ring, and filled Minute circle; Block and Minute states expose an inline numeric field and unit before the menu chevron. The remaining metadata controls are text-only on both platforms. Direction uses its configured color, priority uses red/neutral/green for high/medium/low, and date remains neutral. Hashtags display with `#`, deduplicate case-insensitively, and preserve the first entered casing.
