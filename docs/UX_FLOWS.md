...
On macOS and iOS, `履歴 > タスク` also exposes `記録を追加`. The user chooses an exact historical date/time and either an existing Task occurrence from that day, including a zero-Flow Habit occurrence, or creates a new Task scheduled for that day. Recording Check writes historical completion without inventing Flow. Recording Block or Minute creates a completed independent Flow and rebuilds measured progress from persisted history. Zero-Flow scheduled Tasks remain absent from the actual History summary. The row action with a fixed Task remains available as the faster manual-Flow path. Expanded `履歴 > 方向` ends with `タスクを追加`, which creates a Task with fixed Direction but no Flow. The calendar does not provide direct resize and does not persist a second calendar entity.
