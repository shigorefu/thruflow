...
Inside the Flow task picker, quick creation replaces the picker contents inline instead of opening a nested popover. Closing the composer returns to the picker; creating a Task selects it and closes the picker. This keeps the same capture model in the main window and menu bar without stacking AppKit popover windows.
