...
A manually added calendar Flow uses the existing FlowSession and FlowSegment schema. It is completed immediately, uses its own ID as `seriesID`, stores exact start/end seconds, and applies the same Direction/Todo progress calculation as timer-created history. No manual FlowBreak is created.
