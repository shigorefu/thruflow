...
struct TaskComposerSuggestionPanel<Content: View>: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> TaskComposerSuggestionAnchorView { TaskComposerSuggestionAnchorView() }

    func updateNSView(_ view: TaskComposerSuggestionAnchorView, context: Context) {
        view.update(content: AnyView(content()), isPresented: isPresented, showsBelow: showsBelow)
    }

    static func dismantleNSView(_ view: TaskComposerSuggestionAnchorView, coordinator: ()) {
        view.dismiss()
    }

}

// Keep the AppKit view outside the generic representable. Its erased content
// needs no generic context, and this avoids a Swift 6.3 Release deinit optimizer crash.
final class TaskComposerSuggestionAnchorView: NSView {
